...
//------------------------------------
//Faz com que os players possam adicionar armas ao seus inventarios
//Dando a possibilidade de fazer muniçao infinita por adicionar e retirar essas armas do inventario
//VVVVVVVVVVVVVVVVVVVVVVVVVVVV//
private const bool ALLOW_WEAPON_PICKUP = false;
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^//
...
private class InvControl{
    ...
    private class Inventory{
        ...
        private List<string> iObjectsNames = new List<string>();
        private IObject cObj;
        private IPlayer controller;
        private int size;
        ...
        private Color textColor = new Color((byte)rand.Next(200, 250), (byte)rand.Next(0, 250), 0);
        ...
        public void AddObj(IObject obj){
            ...
            obj.Remove();
            this.iObjectsNames.Add(obj.Name);
            this.currentIndex = this.iObjects.IndexOf(obj);
        ...
        public void DropObj(bool destroy, bool rmv){
            ...
            if(this.iObjects[this.currentIndex] is IPlayer){
                this.cPly = (IPlayer)iObjects[this.currentIndex];
                this.cProfile = cPly.GetProfile();
                this.cPly = Game.CreatePlayer(this.controller.GetWorldPosition() + new Vector2(8f * this.controller.FacingDirection, 6f));
                this.cPly.SetProfile(this.cProfile);
                this.cPly.Kill();
                this.cObj = (IObject)this.cPly;
            }else{
                this.cObj = Game.CreateObject(this.iObjectsNames[this.currentIndex]);
                this.cObj.SetWorldPosition(this.controller.GetWorldPosition() + 
                    new Vector2(this.controller.FacingDirection * 
                        this.cObj.GetAABB().Width, this.cObj.GetAABB().Height/2));
            }
            Game.RemoveCameraFocus(cObj);
            ...
            this.iObjectsNames.Remove(this.iObjectsNames[this.currentIndex]);
        ...
        public void ThrowObj(bool up){
            ...
            this.pickedObjectMass = cObj.GetMass();

            if(this.pickedObjectMass < 0.008f){
                this.pickedObjectMass = 0.008f;
            }
            this.cObj.SetBodyType(BodyType.Dynamic);
            this.cObj.SetLinearVelocity(new Vector2((THROW_POWER/100)/this.pickedObjectMass * 
        ...
        public void NextIndex(){
            if(this.iObjects.Count < 1 || this.iObjects.Count == 1){
                //Utils.PlayTextEffect("Empty Inventory!", this.controller);
                return;
            }
            if(this.currentIndex + 1 == this.iObjects.Count){
                this.currentIndex = 0;
            }
            else{
                this.currentIndex++;
        ...
        public void UpdateTextStatus(){
            ...
            if(!this.controller.IsDead){
                this.text.SetWorldPosition(this.controller.GetWorldPosition() + textAlign);
                this.textShadow.SetWorldPosition(this.controller.GetWorldPosition() + textShadowAlign);
            }
            else {
                this.text.SetWorldPosition(Game.GetBorderArea().TopRight + new Vector2(0f, 120f));
                this.textShadow.SetWorldPosition(Game.GetBorderArea().TopRight + new Vector2(0f, 120f));
            }
            ...
            if(this.iObjects.Count < 1){
                ...
                try{
                    ...
                    this.controlStringArray = this.iObjectsNames[this.currentIndex].Split('0');
...
private class Utils{
    ...
    public static bool IsPowerup(IObjectWeaponItem wpn){
        if(wpn.WeaponItemType == WeaponItemType.Powerup 
            || wpn.WeaponItemType == WeaponItemType.InstantPickup 
                || ALLOW_WEAPON_PICKUP){
            return true;
        }
        return false;
